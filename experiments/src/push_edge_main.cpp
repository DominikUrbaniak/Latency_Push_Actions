...
class RobotPushNode : public rclcpp::Node
{
  ...
  private:
    ...
    void timer_callback()
    {
      ...
      if(abs(get_model_state_euler_delayed.yaw) >= abs(cube_yaw_goal)){
        trigger = 1;
        ...
        if (final_status == std::future_status::ready) {
            RCLCPP_INFO(this->get_logger(), "Object reached goal %f, back up!, %d", cube_yaw_goal, back_up_counter);
            back_up_counter++;
...
int main(int argc, char * argv[])
{
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("push_edge_main");
  srand ( time(NULL) );
  ...
  if (argc == 6) {
    ...
    RCLCPP_INFO(node->get_logger(), "** 5 inputs ** network: %s, target y: %f, tcp_y_init: %f, step_size: %f ", network.c_str(), cube_y_goal, tcp_y_init, step_size);
  }
  else{
    RCLCPP_WARN(node->get_logger(), "Please add either five arguments: 1-distance to approach (0.1), 2-network (ethernet), 3-step size (0.0001), 4-latency mode (0: no delay, 1: introduce latency from network, 2:introduce latency from network AND computation latency, 3:introduce computation latency), 5-computation latency (170)");
    return -1;
  }
  ...
  if(save_params_to_csv){
    ...
    if(set_delay_mode==1){
      ...
      filename = dir_path + std::to_string(secs) + "_" + std::to_string(set_delay_mode) + "mode_"+ std::to_string(control_rate_ms)+ "ms_" + std::to_string(frame_rate_microsec)+ "microsecs_" + std::to_string(computation_latency/1000)+"cl_"+ network.c_str() +"_"+ std::to_string(step_size) + "steps.csv";
    }
    else{
    ...
    myfile << "secs_eef,tcp_x,tcp_y,tcp_z,tcp_yaw,tcp_x_,tcp_y_,trigger,secs_sensed,secs_delayed,latency,cube_x,cube_y,cube_yaw\n";
  }
  executor.spin();
